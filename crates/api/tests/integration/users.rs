...
async fn create_user_returns_201(pool: PgPool) {
    ...
    let response = app
        .post("/api/v1/users", &json!({ "email": "alice@example.com", "password": "secret123" }))
        .await;
...
async fn create_user_returns_400_on_invalid_email(pool: PgPool) {
    ...
    let response = app
        .post("/api/v1/users", &json!({ "email": "not-an-email", "password": "secret123" }))
        .await;
...
async fn create_user_returns_400_on_short_password(pool: PgPool) {
    ...
    let response = app
        .post("/api/v1/users", &json!({ "email": "user@example.com", "password": "short" }))
        .await;
...
async fn get_user_returns_200(pool: PgPool) {
    ...
    let created = body_json(
        app.post("/api/v1/users", &json!({ "email": "get@example.com", "password": "secret123" }))
            .await,
    )
    .await;
...
async fn list_users_cursor_pagination_traverses_all_pages(pool: PgPool) {
    ...
    let body = body_json(app.get("/api/v1/users?limit=2").await).await;
    assert_eq!(body["data"].as_array().unwrap().len(), 2);
    assert_eq!(body["meta"]["has_next_page"], true);
    let cursor = body["meta"]["next_cursor"].as_str().unwrap().to_string();

    let body = body_json(app.get(&format!("/api/v1/users?limit=2&after={cursor}")).await).await;
    assert_eq!(body["data"].as_array().unwrap().len(), 2);
    assert_eq!(body["meta"]["has_next_page"], true);
    let cursor = body["meta"]["next_cursor"].as_str().unwrap().to_string();
